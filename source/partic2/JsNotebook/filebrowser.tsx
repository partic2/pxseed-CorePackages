...
class FileBrowserComponent<P extends {fs:SimpleFileSystem}={fs:SimpleFileSystem}> extends React.Component<P,FileBrowserState>{
    ...
    protected async _askForFileName(initname:string):Promise<string|null>{
    ...
    protected _clipboardFile={
        paths:[] as string[],
        mode:'copy' as 'copy'|'cut'
    }
    protected _clipboardIsCut=false;
    async DoCopy(){
    ...
    async DoPaste(){
        if(this._clipboardFile.mode==='cut'){
            ...
            for(let t1 of this._clipboardFile.paths){
                ...
                if(filetype1==='dir'){
                    await files.copyFileTree(t1,dstpath,{confilctPolicy:'overwrite',srcFs:this.props.fs,destFs:this.props.fs});
                }else{
                    await files.copySingleFile(t1,dstpath,{srcFs:this.props.fs,destFs:this.props.fs});
                }
            }
        }
        this.reloadFileInfo();
    ...
    protected filterRef=React.createRef<HTMLInputElement>();
    ...
    protected onFilterChange(filterText:string){
    ...
    public render(){
        return (<div className={css.flexColumn} style={{height:'100%'}}>
            ...
            {this.renderAction()}
            <input type='text' placeholder='filter' value={this.state.filterText} onInput={(ev)=>this.onFilterChange((ev.target as HTMLInputElement).value)}></input>
...
export namespace __internal__{
    export type FileBrowser=FileBrowserComponent;
    export var FileBrowser=FileBrowserComponent;
    export type WorkspaceFileBrowser=WorkspaceFileBrowser2;
    export var WorkspaceFileBrowser=WorkspaceFileBrowser2;
}